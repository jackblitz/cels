...
- **Return type on same line**: Never place a newline between the return
  type and the function name. A function's return type (including `void`,
  `static void`, `void *`, result codes, etc.) must always be on the same line
...
- `BreakAfterReturnType: None` (and `AlwaysBreakAfterReturnType: None`) ensures
  that the return type stays on the same line as the function name, preventing
  tools and LLMs from breaking `void` or other return types onto their own line.
